...
const Header = ({ isMobileMenuOpen, onToggleMobileMenu, onCloseMobileMenu, userTitle = "Staff" }) => {
...
const StatusTable = ({ requests, selectedTab, userNameParts }) => {
  ...
  const getFeedbackPath = (id) => `/stafffeedback/${id}`;

  // Format the requester's name from the request data
  const formatRequesterName = (request) => {
    // Use the requester's name from the request data if available
    if (request.requester_last_name || request.requester_first_name) {
      return [
        request.requester_last_name,
        request.requester_first_name,
        request.requester_middle_name,
        request.requester_suffix
      ]
        .filter(Boolean)
        .join(", ");
    }
    
    // Fallback to request's name fields if available
    return request.requesting_personnel || "N/A";
  };
          ...
          {requests.length > 0 ? (
            requests.map((request) => (
              <tr key={request.request_id} className="hover:bg-gray-50 even:bg-gray-50 border-b border-gray-400">
                <td className="p-3">{formatRequesterName(request)}</td>
                ...
                {selectedTab === "Done" && (
                  <td className="p-3">
                    <button
                      onClick={() => navigate(`/stafffeedback/${request.request_id}`)}
